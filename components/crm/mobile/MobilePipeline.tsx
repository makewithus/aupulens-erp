...
export default function MobilePipeline() {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex overflow-x-auto pb-2 px-4 gap-2 no-scrollbar shrink-0 border-b border-neutral-800 bg-neutral-900 pt-2">
        {stages.map(s => (
          <button 
            key={s} 
            onClick={() => setActiveStage(s)}
            className={`whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
              activeStage === s ? 'bg-primary text-white' : 'bg-neutral-800 text-neutral-400'
            }`}
          >
            {s}
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-neutral-950">
        {deals.filter(d => d.stage === activeStage).length === 0 ? (
          <div className="text-center text-neutral-500 text-sm mt-10">No deals in this stage.</div>
        ) : deals.filter(d => d.stage === activeStage).map(deal => (
          <div key={deal.id} className="bg-neutral-900 border border-neutral-800 rounded-xl p-4 shadow-sm">
            <div className="flex justify-between items-start mb-2">
              <div>
                <h4 className="font-bold text-sm text-neutral-200">{deal.name}</h4>
            ...
            <div className="mt-3 flex justify-between items-center border-t border-neutral-800 pt-3">
              <span className="text-xs text-neutral-500">Next: Follow Up Call</span>
              <button className="text-xs bg-neutral-800 px-3 py-1 rounded text-neutral-300">Advance</button>
            </div>
          </div>
        ))}
      </div>
