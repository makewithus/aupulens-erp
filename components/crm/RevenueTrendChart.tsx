...
export function RevenueTrendChart() {
  ...
  useEffect(() => {
    ...
    // ── Revenue cumulative S-curve data (growing to ~₹7,50,00,000) ──
    ...
    // Normalize to ₹7,50,00,000
    const targetRevenue = 75000000;
    const uacScale = targetRevenue / uacCum;
    ...
    const yTicks: number[] = [];
    for (let v = 0; v <= yMax; v += 10000000) yTicks.push(v);

    const PAD = { top: 16, right: 60, bottom: 40, left: 10 };
    const color = "#10b981"; // Green for Revenue
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      tooltipValue.innerHTML = `<span class="w-2 h-2 rounded-[1px] inline-block mr-1.5" style="background:${color}"></span> Cumulative Rev <b>₹${d.value.toLocaleString(
        "en-IN"
      )}</b>`;
    ...
    <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-6 font-mono w-full relative">
      <div className="flex justify-between items-center gap-4 mb-4">
        ...
        <div className="text-right">
          <span className="text-[10px] text-neutral-500 uppercase tracking-wider block">Total ARR</span>
          <span className="text-sm font-semibold text-emerald-400">₹7.50 Cr</span>
        </div>
      </div>
