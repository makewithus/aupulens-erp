...
export function CampaignROIChart() {
  ...
  useEffect(() => {
    ...
    // Subsystem nodes representing marketing channels
    const nodes = [
      { name: "Google Ads", lat: 40, lon: -30, color: "#6366f1", size: 8, msgs: "2.4M" },
      { name: "Meta Ads", lat: 25, lon: 45, color: "#e11d48", size: 7, msgs: "1.2M" },
      { name: "LinkedIn Ads", lat: -15, lon: 80, color: "#3b82f6", size: 7, msgs: "1.5M" },
      { name: "Organic Search", lat: 10, lon: -75, color: "#f59e0b", size: 7, msgs: "1.3M" },
      { name: "Email Newsletters", lat: 55, lon: 10, color: "#facc15", size: 9, msgs: "890K" },
      { name: "Direct Visitors", lat: -35, lon: -50, color: "#ec4899", size: 6, msgs: "1.1M" },
      { name: "Partner Referrals", lat: -50, lon: 30, color: "#8b5cf6", size: 7, msgs: "980K" },
      { name: "Affiliates", lat: 15, lon: 130, color: "#22c55e", size: 5, msgs: "420K" },
      { name: "YouTube Videos", lat: -20, lon: -120, color: "#1e3a5f", size: 5, msgs: "350K" },
      { name: "Webinars", lat: 35, lon: -150, color: "#14f195", size: 6, msgs: "610K" },
      { name: "Offline Events", lat: -55, lon: 100, color: "#a78bfa", size: 4, msgs: "210K" },
      { name: "Cold Outreach", lat: 0, lon: 170, color: "#dc2626", size: 5, msgs: "380K" }
    ];
    ...
    function draw() {
      ...
      sortedEdges.forEach(({ a, b }) => {
        ...
        // Curved edge (midpoint lifted off sphere)
        const midP3 = {
          x: (a.p3.x + b.p3.x) / 2,
          y: (a.p3.y + b.p3.y) / 2,
          z: (a.p3.z + b.p3.z) / 2
      ...
      // Tooltip
      if (hoveredNode) {
        tooltip.innerHTML = `<b style="color:${hoveredNode.color}">${hoveredNode.name}</b><br><span style="color:#757575">Total Leads:</span> ${hoveredNode.msgs}`;
  ...
  return (
    <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-6 font-mono w-full relative flex flex-col items-center">
      ...
      <div ref={containerRef} className="relative w-full h-[380px] flex items-center justify-center">
