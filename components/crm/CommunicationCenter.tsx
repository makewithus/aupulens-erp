...
import { Send, MessageSquare, Phone, Mail, Clock, Loader2 } from "lucide-react";
    ...
    <div className="flex flex-col h-full bg-card border border-border rounded-lg">
      <div className="p-4 border-b border-border">
        ...
        {channel === "Email" && (
          <Input 
            ...
            placeholder="Subject..." className="mb-2 bg-background border-border text-sm h-8" 
          />
        )}
        <textarea
          value={message}
          ...
          className="w-full h-24 bg-background border border-border rounded p-2 text-sm text-foreground resize-none focus:outline-none focus:border-border"
        ></textarea>
        
        <div className="flex justify-between items-center mt-2">
          <Button variant="ghost" size="sm" className="h-8 text-xs text-muted-foreground">Use Template</Button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 max-h-[500px]">
        {loading ? (
          ...
          <div className="text-center text-muted-foreground text-sm py-10 border border-dashed border-border rounded">
          ...
          comms.map(comm => (
            <div key={comm._id} className="flex gap-3">
              <div className={`mt-1 p-2 rounded-full h-8 w-8 flex items-center justify-center ${
                comm.direction === 'inbound' ? 'bg-blue-900/30 text-blue-400' : 'bg-accent text-muted-foreground'
              }`}>
              ...
              <div className="flex-1 bg-background border border-border rounded p-3">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <span className="text-xs text-muted-foreground">to {comm.recipient}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-muted-foreground">{new Date(comm.createdAt).toLocaleString()}</span>
                ...
                <div className="text-sm text-foreground whitespace-pre-wrap">{comm.message}</div>
              </div>
            </div>
          ))
        )}
