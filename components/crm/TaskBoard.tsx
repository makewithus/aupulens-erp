...
export default function TaskBoard() {
  ...
  return (
    <div className="flex gap-4 overflow-x-auto pb-4">
      {columns.map(col => (
        <div key={col} className="bg-neutral-900 border border-neutral-800 rounded-lg w-80 flex-shrink-0 flex flex-col h-[70vh]">
          <div className="p-3 border-b border-neutral-800 font-bold bg-neutral-950 rounded-t-lg flex justify-between">
            {col} <Badge variant="secondary">{tasks.filter(t => t.status === col).length}</Badge>
          </div>
          <div className="p-2 flex-1 overflow-y-auto space-y-2">
            {tasks.filter(t => t.status === col).map(t => (
              <div key={t._id} className="bg-neutral-950 p-3 rounded border border-neutral-800 shadow-sm cursor-grab active:cursor-grabbing">
