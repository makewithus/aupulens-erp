...
export function CopyableId({ value, label, className }: { value: string; label?: string; className?: string }) {
  return (
    <button
      ...
      className={cn(
        "font-mono text-xs text-neutral-500 hover:text-neutral-900 dark:hover:text-neutral-100 cursor-pointer underline decoration-dotted underline-offset-2 inline-block align-bottom",
        className,
      )}
    >
      {formatShortId(value)}
