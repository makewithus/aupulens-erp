/**
 * Regression test: HR AI assistant route — tenant AI gating.
...
import { POST } from "@/app/api/hr/ai-assistant/route";

const TENANT_A = "tenant-alpha";

function makeSession(tenantId: string | undefined) {
  return { user: { id: "user-1", role: "hr", tenantId } };
}
...
beforeEach(() => {
  ...
  mockCallClaudeForTenant.mockResolvedValue({ gated: false, text: "HR overview response." });
  mockChatHistoryFindOne.mockReturnValue({ lean: () => Promise.resolve(null) });
  mockChatHistoryFindOneAndUpdate.mockResolvedValue({});
});

describe("hr/ai-assistant — tenant AI gating", () => {
  it("returns 200 with a real response when the tenant is not gated", async () => {
    const res = await POST(makeRequest({ message: "how many employees?" }));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.response).toBe("HR overview response.");
  });
  ...
  it("resolves tenant AI settings using the authenticated tenant", async () => {
    await POST(makeRequest({ message: "how many employees?" }));
