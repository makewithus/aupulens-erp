...
| AI-11 (inventory/COGS) | 4/4 (100%) | `tests/golden/ai11/`. Negative-stock detection with a causing sequence, count variance valued at the tenant's own weighted-average cost (never the standard_price, deliberately set far off), a healthy-inventory must-stay-silent case (positive stock, exact count match, GL ties out, stable margin), and a stale-standard-price valuation anomaly. |
| AI-29 (control monitoring) | 4/4 (100%) | `tests/golden/ai29/`. Three targeted correct-answer cases (`approval_present`, `sod_preparer_approver`, `no_posting_into_locked_period` exceptions) plus one full-period must-stay-silent case asserting zero exceptions across every implemented control at once. |
