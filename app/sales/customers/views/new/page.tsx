...
export default function NewCustomerViewPage() {
  const { data: session } = useSession();
  const router = useRouter();
  const [name, setName] = useState("");
  const [isFavorite, setIsFavorite] = useState(false);
  const [criteria, setCriteria] = useState<Criterion[]>([{ field: "displayName", comparator: "contains", value: "" }]);
  const [selectedColumns, setSelectedColumns] = useState<string[]>(AVAILABLE_CUSTOMER_COLUMNS.slice(0, 3).map((c) => c.key));
  const [availableSearch, setAvailableSearch] = useState("");
  const [visibility, setVisibility] = useState<string>(SALES_VIEW_VISIBILITY.ONLY_ME);
  const [saving, setSaving] = useState(false);

  const availableColumns = AVAILABLE_CUSTOMER_COLUMNS.filter(
    (c) => !selectedColumns.includes(c.key) && c.label.toLowerCase().includes(availableSearch.toLowerCase()),
  );

  const addCriterion = () => setCriteria([...criteria, { field: "displayName", comparator: "contains", value: "" }]);
      ...
      <div className="max-w-4xl mx-auto p-6 space-y-8">
        ...
        <div className="flex items-end gap-4 max-w-lg">
          ...
          <button
            ...
            className="flex items-center gap-1 text-sm pb-2 text-muted-foreground"
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="border border-border/40 rounded-none">
                ...
                <Droppable droppableId="selected">
                  {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} className="min-h-[220px] p-2 space-y-1">
                      <div className="flex items-center gap-2 text-sm px-2 py-1.5 bg-muted/50 border border-border/40 rounded-none font-medium">
                        Name <span className="text-red-500">*</span>
                        <span className="text-xs text-muted-foreground ml-auto">mandatory</span>
                      </div>
                      {selectedColumns.map((key, i) => {
                        const col = AVAILABLE_CUSTOMER_COLUMNS.find((c) => c.key === key);
                        return (
