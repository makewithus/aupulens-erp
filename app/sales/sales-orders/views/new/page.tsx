...
const CRITERIA_FIELDS = [
  ...
  { key: "customerViewed", label: "Customer Viewed" },
  { key: "clientOrderRef", label: "Reference#" },
...
export default function NewSaleOrderViewPage() {
  const { data: session } = useSession();
  const router = useRouter();
  const [name, setName] = useState("");
  const [isFavorite, setIsFavorite] = useState(false);
  const [criteria, setCriteria] = useState<Criterion[]>([{ field: "salesOrderStatus", comparator: "equals", value: "confirmed" }]);
  const [selectedColumns, setSelectedColumns] = useState<string[]>([]);
  const [availableSearch, setAvailableSearch] = useState("");
  const [visibility, setVisibility] = useState<string>(SALES_VIEW_VISIBILITY.ONLY_ME);
  const [saving, setSaving] = useState(false);

  const availableColumns = AVAILABLE_SALE_ORDER_COLUMNS.filter(
    (c) => !selectedColumns.includes(c.key) && c.label.toLowerCase().includes(availableSearch.toLowerCase()),
  );

  const addCriterion = () => setCriteria([...criteria, { field: "salesOrderStatus", comparator: "equals", value: "" }]);
                    ...
                    {(provided) => (
                      <div ref={provided.innerRef} {...provided.droppableProps} className="min-h-[120px] space-y-1">
                        {selectedColumns.map((key, i) => {
                          const col = AVAILABLE_SALE_ORDER_COLUMNS.find((c) => c.key === key);
                          return (
