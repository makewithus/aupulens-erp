...
const PAYMENT_FIELDS = [
  ...
  { key: "paymentAmount", label: "Payment Amount", required: true },
...
export default function ImportPaymentsPage() {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await cachedFetch("/api/sales/payments/import/execute", { method: "POST", body: fd });
      const data = await res.json();
  ...
  const missingRequired = PAYMENT_FIELDS.filter((f) => f.required && !mapping[f.key]);
    ...
    >
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-4xl md:text-[56px] font-black tracking-tighter text-primary">Invoice Payments - Select File</h1>
          <button onClick={() => router.push("/sales/payments")} className="text-muted-foreground hover:text-foreground">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            <p className="text-xs text-muted-foreground">
              Download a{" "}
              <Link href="/api/sales/payments/import/sample?format=csv" className="text-primary underline">
                sample file
              </Link>{" "}
            ...
            <label className="flex items-start gap-2 text-sm">
              ...
              <span>
                <span className="font-medium">Auto-Generate Payment Numbers</span>
                <br />
                <span className="text-xs text-muted-foreground">
                  Payment numbers will be generated automatically according to your settings. Any Payment numbers in
                  the import file will be ignored.
                </span>
              </span>
            </label>
            ...
            <div className="border rounded-none p-4 bg-muted/30 text-sm space-y-1">
              ...
              <ul className="list-disc list-inside text-xs text-muted-foreground space-y-1">
                <li>Date, Customer Name, and Payment Amount are required for every row.</li>
        ...
        {step === 2 && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">Map each column in your file to a payment field.</p>
          ...
          <div className="space-y-4">
            ...
            <div className="overflow-x-auto border border-border/40 rounded-none">
              <Table>
                <TableHeader>
                  <TableRow>
                    {PAYMENT_FIELDS.filter((f) => mapping[f.key]).map((f) => (
            ...
            {result ? (
              <div className="border border-emerald-500/30 rounded-none p-4 bg-card text-sm space-y-1">
                ...
                <Button className="mt-2 font-mono text-[11px] uppercase tracking-wider" onClick={() => router.push("/sales/payments")}>
                  Done
                </Button>
              </div>
            ) : (
