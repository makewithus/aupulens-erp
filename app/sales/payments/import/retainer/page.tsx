...
function autoMap(columns: string[]) {
  ...
  for (const field of RETAINER_PAYMENT_FIELDS) {
...
export default function ImportRetainerPaymentsPage() {
  ...
  const missingRequired = RETAINER_PAYMENT_FIELDS.filter((f) => f.required && !mapping[f.key]);
  ...
  return (
    <DashboardLayout
      ...
      pageName="Import Retainer Payments"
      breadcrumbs={[
    ...
    >
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-4xl md:text-[56px] font-black tracking-tighter text-primary">Retainer Payments - Select File</h1>
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            <p className="text-xs text-muted-foreground">
              Download a{" "}
              <Link href="/api/sales/payments/import-retainer/sample?format=csv" className="text-primary underline">
                sample file
              </Link>{" "}
        ...
        {step === 2 && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">Map each column in your file to a retainer payment field.</p>
            <div className="space-y-2">
              {RETAINER_PAYMENT_FIELDS.map((field) => (
          ...
          <div className="space-y-4">
            ...
            <div className="overflow-x-auto border border-border/40 rounded-none">
              <Table>
                <TableHeader>
                  <TableRow>
                    {RETAINER_PAYMENT_FIELDS.filter((f) => mapping[f.key]).map((f) => (
