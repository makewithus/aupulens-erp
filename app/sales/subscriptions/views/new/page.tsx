...
const CRITERIA_FIELDS = [
  ...
  { key: "metered", label: "Metered Billing" },
  { key: "autoRenew", label: "Auto Renew" },
  { key: "unbilledCharges", label: "Unbilled Charges" },
...
export default function NewSubscriptionViewPage() {
  const { data: session } = useSession();
  const router = useRouter();
  const [name, setName] = useState("");
  const [isFavorite, setIsFavorite] = useState(false);
  const [criteria, setCriteria] = useState<Criterion[]>([{ field: "status", comparator: "equals", value: "active" }]);
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    AVAILABLE_SUBSCRIPTION_COLUMNS.slice(0, 3).map((c) => c.key),
  );
  const [availableSearch, setAvailableSearch] = useState("");
  const [visibility, setVisibility] = useState<string>(SALES_VIEW_VISIBILITY.ONLY_ME);
  const [saving, setSaving] = useState(false);

  const columnPool = AVAILABLE_SUBSCRIPTION_COLUMNS;
  const availableColumns = columnPool.filter(
                  ...
                  {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} className="min-h-[220px] p-2 space-y-1">
                      ...
                      {selectedColumns.map((key, i) => {
                        const col = AVAILABLE_SUBSCRIPTION_COLUMNS.find((c) => c.key === key);
                        return (
