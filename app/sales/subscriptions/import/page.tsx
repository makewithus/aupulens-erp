...
const SUBSCRIPTION_FIELDS = [
  ...
  { key: "billingCycles", label: "Billing Cycles" },
...
export default function ImportSubscriptionsPage() {
  ...
  const missingRequired = SUBSCRIPTION_FIELDS.filter((f) => f.required && !mapping[f.key]);
    ...
    >
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-4xl md:text-[56px] font-black tracking-tighter text-primary">Subscriptions - Select File</h1>
          <button onClick={() => router.push("/sales/subscriptions")} className="text-muted-foreground hover:text-foreground">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            <p className="text-xs text-muted-foreground">
              Download a{" "}
              <Link href="/api/sales/subscriptions/import/sample?format=csv" className="text-primary underline">
                sample file
              </Link>{" "}
            ...
            <label className="flex items-start gap-2 text-sm">
              ...
              <span>
                <span className="font-medium">Auto-Generate Subscriptions Numbers</span>
                <br />
                <span className="text-xs text-muted-foreground">
                  Subscriptions numbers will be generated automatically according to your settings. Any Subscriptions
            ...
            <label className="flex items-start gap-2 text-sm">
              <Checkbox checked={mapAddresses} onCheckedChange={(v) => setMapAddresses(!!v)} className="mt-0.5" />
              <span>
                <span className="font-medium">Map the customers&apos; addresses in the import file to their customer record.</span>
                <br />
                <span className="text-xs text-muted-foreground">
                  The customer&apos;s name from the import file is used to check whether the customer already exists.
                  If it exists, the address from the subscription rows is mapped onto that customer record.
                </span>
              </span>
            </label>
        ...
        {step === 2 && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">Map each column in your file to a subscription field.</p>
          ...
          <div className="space-y-4">
            ...
            <div className="overflow-x-auto border border-border/40 rounded-none">
              <Table>
                <TableHeader>
                  <TableRow>
                    {SUBSCRIPTION_FIELDS.filter((f) => mapping[f.key]).map((f) => (
            ...
            {result ? (
              <div className="border border-emerald-500/30 rounded-none p-4 bg-card text-sm space-y-1">
                ...
                <Button className="mt-2 font-mono text-[11px] uppercase tracking-wider" onClick={() => router.push("/sales/subscriptions")}>
                  Done
                </Button>
              </div>
            ) : (
