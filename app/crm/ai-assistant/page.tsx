...
export default function CRMAIAssistantPage() {
  ...
  const fetchChatHistory = async () => {
    try {
      ...
      const res = await fetch("/api/crm/chat-history");
      if (res.ok) {
  ...
  const deleteChat = async (chatId: string, e: React.MouseEvent) => {
    ...
    try {
      const res = await fetch(`/api/crm/chat-history?chatId=${chatId}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // for THIS module's forms (lead, account, opportunity, case, campaign,
    // task, contact, quote...) or any OTHER module's — extract the fields and
    // take the user straight to the real, pre-filled form. Falls through to
    // normal Q&A otherwise.
    try {
    ...
    try {
      ...
      if (isFirstMessage) {
        ...
        const saveRes = await fetch("/api/crm/chat-history", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        await fetch("/api/crm/chat-history", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ chatId: currentChatId, messages: allMsgs }),
        });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex h-[calc(100vh-260px)] min-h-[520px] gap-0">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-2 p-3 border-b border-border/40">
            ...
            <h2 className="font-bold text-foreground">CRM AI Assistant</h2>
            <span className="text-xs text-muted-foreground">Ask about leads, accounts, opportunities, cases, campaigns, or tasks — or ask it to create one</span>
          </div>
            ...
            {messages.length === 0 && (
              <div className="flex items-center justify-center h-full">
                <div className="text-center max-w-md">
                  ...
                  <h3 className="font-bold text-lg text-foreground mb-2">CRM AI Assistant</h3>
                  <p className="text-sm text-muted-foreground">
                    Ask me about leads, accounts, the pipeline, cases, campaigns, or tasks — or paste/describe details and I&apos;ll prepare a create form for you (lead, opportunity, case, campaign, task, and more).
                  </p>
          ...
          <div className="p-4 border-t border-border/40">
            ...
            <form onSubmit={handleSubmit} className="flex gap-2 items-end">
              <Textarea
                ...
                onPaste={handlePaste}
                placeholder="Ask about leads, accounts, pipeline, cases, campaigns..."
