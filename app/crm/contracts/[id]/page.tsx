...
import {
  ShieldAlert, HeartPulse, RefreshCw, Calendar, FileCheck, ArrowRight, CheckCircle2, Loader2
} from "lucide-react";
    ...
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="bg-card p-6 rounded-lg border border-border flex flex-col gap-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
          <div className={`p-3 rounded-md border ${CHURN_COLOR[data.churn_risk] || 'border-border bg-background'}`}>
          ...
          <div className={`p-3 rounded-md border ${HEALTH_COLOR[healthCategory] || 'border-border bg-background'}`}>
            <div className="flex items-center gap-1.5 mb-1 opacity-80 text-xs uppercase font-bold">
          ...
          <div className="p-3 rounded-md border border-border bg-background">
            ...
            <div className={`text-xl font-bold ${new Date(data.end_date) < new Date() ? 'text-red-400' : 'text-foreground'}`}>
          ...
          <div className="p-3 rounded-md border border-border bg-background">
            ...
            <div className="text-xl font-bold text-foreground">{data.auto_renew ? "Enabled" : "Disabled"}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-4 border-b border-border">
        <button className={`pb-2 px-1 text-sm font-medium ${tab === 'details' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setTab('details')}>Details & Terms</button>
        <button className={`pb-2 px-1 text-sm font-medium ${tab === 'documents' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setTab('documents')}>Documents</button>
        <button className={`pb-2 px-1 text-sm font-medium ${tab === 'renewals' ? 'text-primary border-b-2 border-primary' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setTab('renewals')}>Renewal Activity</button>
      </div>
      ...
      {tab === 'details' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-card border border-border p-6 rounded-lg space-y-4">
            <h2 className="text-lg font-bold border-b border-border pb-2">Contract Information</h2>
            ...
            <h2 className="text-lg font-bold border-b border-border pb-2 mt-6">Linked Records</h2>
            <div className="space-y-3">
              {data.opportunity_id && (
                <div className="bg-background p-3 rounded border border-border flex justify-between items-center">
                  <div>
                    <p className="text-xs text-muted-foreground">Originating Opportunity</p>
              ...
              {data.quote_id && (
                <div className="bg-background p-3 rounded border border-border flex justify-between items-center">
                  <div>
                    <p className="text-xs text-muted-foreground">Signed Quote</p>
          ...
          <div className="bg-card border border-border p-6 rounded-lg flex flex-col">
            <h2 className="text-lg font-bold border-b border-border pb-2 mb-4">Terms & Conditions</h2>
            <div className="flex-1 bg-background border border-border rounded p-4 text-sm text-foreground whitespace-pre-wrap overflow-y-auto max-h-[400px]">
              {data.terms || <span className="text-muted-foreground italic">No terms specified. Standard terms apply.</span>}
            </div>
            
            {data.notes && (
              <>
                <h2 className="text-lg font-bold border-b border-border pb-2 mt-6 mb-4">Internal Notes</h2>
                <div className="bg-background border border-border rounded p-4 text-sm text-foreground whitespace-pre-wrap">
                  {data.notes}
                </div>
              </>
            )}
      ...
      {tab === 'renewals' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-card border border-border p-6 rounded-lg">
            <h2 className="text-lg font-bold mb-4 border-b border-border pb-2">Renewal Status</h2>
            ...
            {data.renewal_opportunity_id ? (
              <div className="bg-blue-900/10 border border-blue-900/50 p-4 rounded-lg">
                ...
                <p className="text-sm text-foreground">{data.renewal_opportunity_id.deal_name}</p>
                <div className="mt-3 flex justify-between items-center text-sm font-mono">
                  ...
                  <span className="text-muted-foreground">{data.renewal_opportunity_id.probability}% Prob</span>
                </div>
                <Button className="w-full mt-4 bg-accent hover:bg-accent text-accent-foreground text-sm h-8" asChild>
              ...
              <div className="text-center py-10 bg-background border border-border rounded-lg">
                ...
                <p className="text-muted-foreground text-sm">No renewal opportunity active.</p>
                {data.renewal_status === 'Not Started' && (
            ...
            <div className="mt-6 space-y-2">
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Automated Reminders</h3>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className={`p-2 rounded border ${data.reminder_90_sent ? 'border-green-900/50 bg-green-900/10 text-green-400' : 'border-border bg-background text-muted-foreground'}`}>
                  <div className="flex justify-between"><span>90 Days</span> {data.reminder_90_sent && <CheckCircle2 className="w-4 h-4" />}</div>
                </div>
                <div className={`p-2 rounded border ${data.reminder_60_sent ? 'border-green-900/50 bg-green-900/10 text-green-400' : 'border-border bg-background text-muted-foreground'}`}>
                  <div className="flex justify-between"><span>60 Days</span> {data.reminder_60_sent && <CheckCircle2 className="w-4 h-4" />}</div>
                </div>
                <div className={`p-2 rounded border ${data.reminder_30_sent ? 'border-orange-900/50 bg-orange-900/10 text-orange-400' : 'border-border bg-background text-muted-foreground'}`}>
                  <div className="flex justify-between"><span>30 Days</span> {data.reminder_30_sent && <CheckCircle2 className="w-4 h-4" />}</div>
                </div>
                <div className={`p-2 rounded border ${data.reminder_7_sent ? 'border-red-900/50 bg-red-900/10 text-red-400' : 'border-border bg-background text-muted-foreground'}`}>
          ...
          <div className="bg-card border border-border p-6 rounded-lg">
            <h2 className="text-lg font-bold mb-4 border-b border-border pb-2">Activity Trail</h2>
