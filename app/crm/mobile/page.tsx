...
export default function MobileCRMHome() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-md mx-auto bg-black text-white relative overflow-hidden pb-16 md:pb-0 md:h-[800px] md:border md:border-neutral-800 md:mt-8 md:rounded-3xl">
      {/* Header */}
      <div className="flex justify-between items-center p-4 border-b border-neutral-800 bg-neutral-900 sticky top-0 z-10">
        ...
        {activeTab === "dashboard" && (
          <div className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              <Button variant="outline" className="h-20 flex flex-col items-center justify-center gap-2 bg-neutral-900 border-neutral-800" onClick={() => setActiveTab("lead")}>
                <UserPlus className="w-6 h-6 text-blue-400" /> <span className="text-xs">New Lead</span>
              </Button>
              <Button variant="outline" className="h-20 flex flex-col items-center justify-center gap-2 bg-neutral-900 border-neutral-800" onClick={() => setActiveTab("call")}>
                <Phone className="w-6 h-6 text-green-400" /> <span className="text-xs">Log Call</span>
              </Button>
              <Button variant="outline" className="h-20 flex flex-col items-center justify-center gap-2 bg-neutral-900 border-neutral-800" onClick={() => setActiveTab("voice")}>
                <Mic className="w-6 h-6 text-purple-400" /> <span className="text-xs">Voice Note</span>
              </Button>
              <Button variant="outline" className="h-20 flex flex-col items-center justify-center gap-2 bg-neutral-900 border-neutral-800" onClick={() => setActiveTab("pipeline")}>
                <AlignLeft className="w-6 h-6 text-orange-400" /> <span className="text-xs">Pipeline</span>
              </Button>
            </div>
            
            <div className="mt-6">
              <h3 className="font-semibold text-sm mb-3 text-neutral-400">Quick Actions (Nearby)</h3>
              <QuickFollowUp />
            </div>
          </div>
        )}
      ...
      {/* Bottom Nav Bar */}
      <div className="absolute bottom-0 w-full bg-neutral-900 border-t border-neutral-800 flex justify-around p-3 pb-safe z-10">
        <Button variant="ghost" className="flex flex-col items-center gap-1 h-auto py-1" onClick={() => setActiveTab("dashboard")}>
          <Search className={`w-5 h-5 ${activeTab === 'dashboard' ? 'text-primary' : 'text-neutral-500'}`} />
        ...
        <Button variant="ghost" className="flex flex-col items-center gap-1 h-auto py-1" onClick={() => setActiveTab("pipeline")}>
          <AlignLeft className={`w-5 h-5 ${activeTab === 'pipeline' ? 'text-primary' : 'text-neutral-500'}`} />
          <span className="text-[10px] text-neutral-500">Deals</span>
        </Button>
        <Button variant="ghost" className="flex flex-col items-center gap-1 h-auto py-1">
          <MapPin className="w-5 h-5 text-neutral-500" />
          <span className="text-[10px] text-neutral-500">Visits</span>
        </Button>
      </div>
    </div>
  );
