...
import { Clock, AlertTriangle, CheckCircle2, User, Building, Loader2 } from "lucide-react";
...
export default function CaseDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col md:flex-row justify-between md:items-center bg-card p-6 rounded-lg border border-border gap-4">
        <div>
        ...
        <div className="col-span-2 space-y-6">
          {data.status === 'Resolved' && !data.satisfaction_score && (
            <div className="bg-blue-900/20 border border-blue-800 p-6 rounded-lg">
              ...
              <input type="text" placeholder="Comments..." className="w-full bg-background p-2 rounded border border-border mb-4 text-sm" value={satComment} onChange={e => setSatComment(e.target.value)} />
          ...
          <Tabs defaultValue="timeline">
            <TabsList className="bg-card border border-border w-full flex overflow-x-auto">
            ...
            <TabsContent value="timeline" className="bg-card border border-border p-6 rounded-lg mt-4">
              <ActivityTimeline linkedRecordId={params.id} />
            </TabsContent>
            <TabsContent value="communications" className="bg-card border border-border p-6 rounded-lg mt-4">
            ...
            <TabsContent value="tasks" className="p-4 bg-card border border-border mt-4 rounded-lg">
              <CaseTasks caseId={params.id} />
            </TabsContent>
            <TabsContent value="escalations" className="bg-card border border-border p-6 rounded-lg mt-4">
        ...
        <div className="space-y-6">
          <div className="bg-card border border-border p-6 rounded-lg">
            ...
            <div className="space-y-3">
              <div><p className="text-xs text-muted-foreground">Description</p><p className="text-sm whitespace-pre-wrap">{data.description || 'No description provided.'}</p></div>
              <div><p className="text-xs text-muted-foreground">Category</p><p className="text-sm">{data.category || '-'}</p></div>
              <div><p className="text-xs text-muted-foreground">Subcategory</p><p className="text-sm">{data.subcategory || '-'}</p></div>
            </div>
          </div>
          ...
          <div className="bg-card border border-border p-6 rounded-lg">
            ...
            <div className="space-y-4">
              ...
              {data.contact_id ? (
                ...
                <p className="text-sm text-muted-foreground">No primary contact linked.</p>
              )}
            </div>
          </div>
