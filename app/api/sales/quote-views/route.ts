...
import { SYSTEM_VIEW_DEFINITIONS } from "@/lib/sales/quoteViews";

async function ensureSystemViews(tenantId: string) {
  const existing = await SalesView.countDocuments({ tenantId, entityType: "quotes", isSystem: true });
...
export async function GET() {
  try {
    ...
    const views = await SalesView.find({ tenantId, entityType: "quotes" })
      .sort({ isSystem: -1, createdAt: 1 })
      .lean();

    return NextResponse.json({ success: true, data: views });
  } catch (error: any) {
    console.error("Quote views GET error:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("Quote views POST error:", error);
    return NextResponse.json({ success: false, message: error.message }, { status: 500 });
  }
}
