...
export async function GET() {
  try {
    ...
    const prefix = await getDefaultPrefix(tenantId, SALES_DOCUMENT_TYPE.SUBSCRIPTION);
    const counter = await Counter.findOne({ tenantId, key: `invoice:subscription:${prefix}` }).lean();
    ...
    console.error("Subscription next-number GET error:", error);
