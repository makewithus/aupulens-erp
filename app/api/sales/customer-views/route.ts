...
import { SYSTEM_VIEW_DEFINITIONS } from "@/lib/sales/customerViews";
...
export async function GET() {
  try {
    ...
    const views = await SalesView.find({ tenantId, entityType: "customers" })
      .sort({ isSystem: -1, createdAt: 1 })
      .lean();

    return NextResponse.json({ success: true, data: views });
  } catch (error: any) {
    console.error("Customer views GET error:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("Customer views POST error:", error);
    return NextResponse.json({ success: false, message: error.message }, { status: 500 });
  }
}
