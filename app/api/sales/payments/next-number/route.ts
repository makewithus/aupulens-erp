...
export async function GET() {
  try {
    ...
    const prefix = await getDefaultPrefix(tenantId, SALES_DOCUMENT_TYPE.PAYMENT);
    const counter = await Counter.findOne({ tenantId, key: `invoice:payment:${prefix}` }).lean();
    ...
    console.error("Payment next-number GET error:", error);
