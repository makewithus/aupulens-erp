...
// Backs "Configure Payment# Preferences" — same shape as the Sales Order/
// Quote/Subscription number-settings routes.
export async function POST(request: NextRequest) {
  try {
    ...
    if (!row) {
      ...
      await clearOtherDefaults(tenantId, SALES_DOCUMENT_TYPE.PAYMENT, DOCUMENT_PREFIX_KIND.PREFIX, row.id);
      row.isDefault = true;
      await row.save();
    }

    await Counter.findOneAndUpdate(
      { tenantId, key: `invoice:payment:${prefix}` },
    ...
    console.error("Payment number-settings POST error:", error);
