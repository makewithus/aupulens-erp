...
const SAMPLE_INVOICE = {
  ...
  customerId: {
    header: { name: "Sample Customer Pvt. Ltd." },
  ...
  try {
    ...
    const ctx = await buildTemplateContext({
      ...
      company: {
        ...
        address: [orgSettings.addressLine1, orgSettings.city].filter(Boolean).join(", "),
        state: orgSettings.state || "Maharashtra",
      },
      bank: { accountName: "Business Current Account", accountNumber: "0123456789", bankName: "Sample Bank", ifsc: "SMPL0000123" },
      signatureUrl: null,
